...
if __name__ == "__main__":
    ...
    with open(text_list_file_path, "rb") as f:
        ...
        for idx, line in enumerate(lines):
            ...
            clean_fname = 'clean_fileid_' + fname.split('_fileid_')[1] + '.wav'
            cmd = 'cp vox/wav/' + clean_fname + dst_dir + '/' + clean_fname + '_original_clean.wav ' + dst_dir 
            os.system(cmd)

            mel_fname = vox_dir + '/festival/falcon_mspec/' + fname + '.feats.npy'
            mel = np.load(mel_fname) 
            waveform = vocoder(model, mel[0:500])
