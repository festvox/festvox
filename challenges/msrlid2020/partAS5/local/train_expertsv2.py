"""Trainining script for Tacotron speech synthesis model. Uses latents
...
if __name__ == "__main__":
    ...
    latents_train, latents_val, latent_ids = get_cat_feats('melvqvae4alatents')

    # Dataset and Dataloader setup
    trainset = LIDlatentsDataset(latents_train, lid_train, fnames_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=hparams.num_workers, shuffle=True,
        collate_fn=collate_fn_lidlatents, pin_memory=hparams.pin_memory)

    valset = LIDlatentsDataset(latents_val, lid_val, fnames_val)
    ...
    # Model
    model = LIDMixtureofExpertslatentsattentionconv()
    model = model.cuda()
