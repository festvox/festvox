...
from util import *
from model import LIDSeq2SeqMixtureofExperts_alltimesteps
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, mel, fname) in tqdm(enumerate(train_loader)):
            ...
            x_stretched = x.unsqueeze(1).expand(val_outputs.shape[0], val_outputs.shape[1]) 
            #print("Shape of val_outputs and x_stretched: ", val_outputs.shape, x.shape, x_stretched.shape)

            # Loss
            loss = criterion(val_outputs.contiguous().view(-1, 2), x_stretched.contiguous().view(-1))
...
if __name__ == "__main__":
    ...
    # Model
    model = LIDSeq2SeqMixtureofExperts_alltimesteps(39)
    model = model.cuda()
