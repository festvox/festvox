"""Trainining script for MAML for LID  v3: Theta is CPC. Phi is Experts v2.
...
def validate_model_full(model, val_loader):
     ...
     log_value("validation loss (per epoch)", averaged_loss, global_epoch_finetuning)
     h.write("Validation Loss after epoch " + str(global_epoch_finetuning) + ': '  + format(running_loss / (len(train_loader))) + '\n' + '\n')
     h.close()
     recall = get_metrics(y_pred, y_true)
     log_value("Unweighted Recall per epoch", recall, global_epoch_finetuning)
...
def finetune_train(model, train_loader, val_loader, optimizer,
    ...
    global global_step_finetuning, global_epoch_finetuning #, model
    ...
    while global_epoch_finetuning < nepochs:
        ...
        for step, (x, mel, fname) in tqdm(enumerate(train_loader)):
            ...
            if global_step_finetuning % checkpoint_interval == 0:
               ...
               save_checkpoint(
                    model, optimizer, global_step_finetuning, checkpoint_dir, global_epoch_finetuning, ema=ema)
...
def meta_train(theta_model, phi_model, train_loader, val_loader, optimizer, optimizer_maml,
    ...
    running_loss_phi = 0.
    criterion = nn.CrossEntropyLoss()
    global global_step, global_epoch #, theta_model, phi_model
    #validate_model(model, val_loader)
...
if __name__ == "__main__":
    ...
    # Model
    phi_model = LIDSeq2SeqMixtureofExpertsv2(39)
    ...
    try:
       print("Meta Training")
       #theta_model, phi_model = meta_train(theta_model, phi_model, train_loader, val_loader, optimizer, optimizer_maml,
