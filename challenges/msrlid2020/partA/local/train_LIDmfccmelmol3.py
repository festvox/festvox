...
def validate_model(model, val_loader):
     ...
     with torch.no_grad():
       for step, (mfcc, mfcc_lengths, mel, mol, lid, fname) in enumerate(val_loader):
          ...
          logits, vq_penalty, encoder_penalty, entropy = model.forward_eval(mfcc, mel, mol)
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (mfcc, mfcc_lengths, mel, mol, lid, fnames) in tqdm(enumerate(train_loader)):
            ...
            logits, vq_penalty, encoder_penalty, entropy, logits_reconstruction, targets_reconstruction = model(mfcc, mel, mol, mfcc_lengths=sorted_lengths)

            # Loss
            #print("Shape of logits and lid: ", logits.shape, lid.shape)
            lid_loss = criterion(logits, lid)
            reconstruction_loss = criterion_reconstruction(logits_reconstruction.transpose(1,2), targets_reconstruction)
            encoder_weight = 0.01 * min(1, max(0.1, global_step / 1000 - 1)) # https://github.com/mkotha/W$
            loss = lid_loss + vq_penalty + encoder_penalty * encoder_weight + reconstruction_loss
