...
from model import DurationAcousticModel
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_loss = 0.
        for step, (x, mel, y) in tqdm(enumerate(train_loader)):
            ...
            if use_multigpu:
               ...
               mel_outputs, linear_outputs, attn = outputs[0], outputs[1], outputs[2]
                ...
                mel_outputs, linear_outputs = model(x, mel)
...
if __name__ == "__main__":
    ...
    tones_train = categorical_datasource( vox_dir + '/' + 'fnames.train', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name)
    ...
    trainset = PhonesNDurationsDataset(X_train, tones_train, Mel_train, Y_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=4, shuffle=True,
        collate_fn=collate_fn_phonesNdurations, pin_memory=hparams.pin_memory)

    # Model
    model = DurationAcousticModel(n_vocab=1+ len(ph_ids))
    model = model.cuda()
