...
def tts(model, text, tones):
    ...
    mel_outputs, linear_outputs, alignments = model(sequence, tones)
...
if __name__ == "__main__":
    ...
    with open(text_list_file_path, "rb") as f:
        ...
        for idx, line in enumerate(lines):
            ...
            for l in h:
                l = l.split('\n')
            text = ' '.join(k for k in l)
            #text = '< ' + text + ' >'

            # Load tones
            tones_fname = vox_dir + '/festival/falcon_tones/' + fname + '.feats'
            h = open(tones_fname)
            for l in h:
                l = l.split('\n')
            tones = ' '.join(k for k in l)
            #tones = '0 ' + tones + ' 0'

            print(text, tones, fname)
            #sys.exit()
            text = [phids[l] for l in text.split()]
            tones = [toneids[l] for l in tones.split()]
            waveform, alignment, _ = tts(model, text, tones)
