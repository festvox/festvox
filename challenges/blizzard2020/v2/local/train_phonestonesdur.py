...
from model import DurationTonesAcousticModel
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_loss = 0.
        for step, (x, t, mel, y) in tqdm(enumerate(train_loader)):
            ...
            x, t, mel, y = Variable(x), Variable(t), Variable(mel), Variable(y)
            if use_cuda:
                x, t, mel, y = x.cuda(), t.cuda(), mel.cuda(), y.cuda()
 
            #print("Shapes: ", x.shape, mel.shape, y.shape)
            #sys.exit()
            ...
            if use_multigpu:
               ...
               mel_outputs, linear_outputs, attn = outputs[0], outputs[1], outputs[2]
                ...
                mel_outputs, linear_outputs = model(x, t, mel)

            # Loss
            #print("Shapes of mel_outputs, mel: ", mel_outputs.shape, mel.shape)
...
if __name__ == "__main__":
    ...
    with open(vox_dir + '/' + 'etc/ids_tones.json') as  f:
       tone_ids = json.load(f)

    tone_ids = dict(tone_ids)

    idsdict_file = checkpoint_dir + '/ids_tones.json'

    with open(idsdict_file, 'w') as outfile:
       json.dump(tone_ids, outfile)


    feats_name = 'phones'
    X_train = categorical_datasource( vox_dir + '/' + 'fnames.train.2000', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name, ph_ids)

    feats_name = 'tones'
    tones_train = categorical_datasource( vox_dir + '/' + 'fnames.train.2000', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name, tone_ids)

    feats_name = 'durations'
    durations_train = categorical_datasource( vox_dir + '/' + 'fnames.train.2000', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name)

    feats_name = 'lspec'
    Y_train = float_datasource(vox_dir + '/' + 'fnames.train.2000', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name)

    feats_name = 'mspec'
    Mel_train = float_datasource(vox_dir + '/' + 'fnames.train.2000', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name)

    # Dataset and Dataloader setup
    trainset = PhonesTonesNDurationsDataset(X_train, tones_train, durations_train, Mel_train, Y_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=4, shuffle=True,
        collate_fn=collate_fn_phonestonesNdurations, pin_memory=hparams.pin_memory)

    # Model
    model = DurationTonesAcousticModel(n_vocab=1+ len(ph_ids), n_tones = 1 + len(tone_ids))
    model = model.cuda()
