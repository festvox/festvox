...
from model import TacotronOneSeqwiseTones as Tacotron
        ...
        for step, (x, tones, input_lengths, mel, y) in tqdm(enumerate(train_loader)):
            ...
            if use_multigpu:
               ...
               mel_outputs, linear_outputs, attn = outputs[0], outputs[1], outputs[2]
                ...
                mel_outputs, linear_outputs, attn = model(x, tones, mel, input_lengths=sorted_lengths)

            # http://www.erogol.com/gradual-training-with-tacotron-for-faster-convergence/
            #if global_step == 1000:
            #   hparams.outputs_per_step = 5 
            #   print("Reduced number of outputs per step to 5") 

            #if global_step == 10000:
            #   hparams.outputs_per_step = 3
            #   print("Reduced number of outputs per step to 3") 
...
if __name__ == "__main__":
    ...
    trainset = PhonesNTonesDataset_dict(X_train, tones_train, Mel_train, Y_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=hparams.num_workers, shuffle=True,
        collate_fn=collate_fn_phonesNtones, pin_memory=hparams.pin_memory)

    ph_ids = trainset.return_phoneids()
    idsdict_file = checkpoint_dir + '/ids_phones.json'
    ...
    # Model
    model = Tacotron(n_vocab=1+ len(ph_ids), n_tones = len(tone_ids) + 1, 
                     embedding_dim=256,
