"""Trainining script for Mandarin speech synthesis model. Fine float used for prediction of fine
...
def train(model, train_loader, val_loader, optimizer,
    ...
    criterion_fine = DiscretizedMixturelogisticLoss() 
    ...
    while global_epoch < nepochs:
        ...
        for step, (mel, coarse, coarse_float, fine, fine_float) in tqdm(enumerate(train_loader)):
            ...
            input_lengths = torch.zeros(mel.shape[0]) + coarse.shape[1] - 1
            coarse_loss = criterion(coarse_logits.contiguous().view(-1, 256), coarse_targets.contiguous().view(-1))
            #fine_loss = criterion_fine(fine_logits.transpose(1,2), fine_targets.view(mel.shape[0], -1, 1), lengths = input_lengths.long(), max_len = coarse.shape[1] - 1)
...
if __name__ == "__main__":
    ...
    if finetune:
       ...
       model.upsample_network = model3.upsample_network
       model.hidden2coarse_hidden = model3.hidden2coarse_hidden
       model.hidden2fine_hidden = model3.hidden2fine_hidden
       model.coarse_hidden2logits_coarse = model3.coarse_hidden2logits_coarse
       model.fine_hidden2logits_fine = model3.fine_hidden2logits_fine
       model.joint_encoder = model3.joint_encoder 
