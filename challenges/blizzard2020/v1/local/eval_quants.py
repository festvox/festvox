...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (mel, coarse, coarse_float, fine, fine_float) in tqdm(enumerate(train_loader)):
            ...
            coarse_outputs = model(mel, coarse, coarse_float, fine, fine_float)

            # Loss
            #print("Shape of logits and targets: ", coarse_outputs.shape, coarse.shape)
            coarse_loss = criterion(coarse_outputs.contiguous().view(-1, 256), coarse.contiguous().view(-1))
            ...
            c_pred = torch.argmax(coarse_outputs[0,:,:], dim=-1).to(torch.float32) * 256 / 32767.5 - 1.0
            c_true = c_true.detach().cpu().numpy()
            c_pred = c_pred.detach().cpu().numpy()
            print("Shapes of c_true and c_pred: ", c_true.shape, c_pred.shape) 
            write("example.wav", 16000, c_true)
