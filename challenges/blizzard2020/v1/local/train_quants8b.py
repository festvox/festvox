"""Trainining script for Mandarin speech synthesis model. 2 encoders
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, (mel, x) in tqdm(enumerate(train_loader)):
            ...
            logits1, logits2, targets = model(mel, x)

            # Loss
            l = x.shape[1]
            loss1 = criterion(logits1.transpose(1,2), targets)
            loss2 = criterion(logits2.transpose(1,2), targets)
            loss = loss1 + loss2
            #print(loss)
            ...
            running_loss_mol1 += loss1.item()
            running_loss_mol2 += loss2.item()

        averaged_loss = running_loss / (len(train_loader))
        log_value("loss (per epoch)", averaged_loss, global_epoch)
        h.write("Loss after epoch " + str(global_epoch) + ': '  + format(running_loss / (len(train_loader))) 
                + "MoL Loss 1 : "  + format(running_loss_mol1 / (len(train_loader))) 
                + "MoL Loss 2 : "  + format(running_loss_mol2 / (len(train_loader))) 
                +  '\n')
        h.close() 
        #sys.exit()
...
if __name__ == "__main__":
    ...
    if finetune:
       ...
       model.joint_encoder1 = model8.joint_encoder 
       model.hidden2linear = model8.hidden2linear
       #model.linear2logits = model8.linear2logits
