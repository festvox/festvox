...
from model import TacotronOnekothamspec as Tacotron
...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_loss = 0.
        for step, (x, input_lengths, mel) in tqdm(enumerate(train_loader)):
            ...
            mel_outputs, attn = model(x, mel, input_lengths=sorted_lengths)
            ...
            if global_step > 0 and global_step % hparams.save_states_interval == 0:
               save_alignment(global_step, attn, checkpoint_dir) 
...
if __name__ == "__main__":
    ...
    trainset = Blzrd2020Dataset(X_train, Mel_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=hparams.num_workers, shuffle=True,
        collate_fn=collate_fn_mspeckotha, pin_memory=hparams.pin_memory)
    
    ## Ok champion, tell me where you are using this  
    valset = Blzrd2020Dataset(X_val, Mel_val)
