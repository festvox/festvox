...
from model import ValenceSeq2Seq_CPCLoss 
...
def train(model, train_loader, val_loader, optimizer,
    ...
    contrastive_criterion = nn.L1Loss()
    ...
    while global_epoch < nepochs:
        ...
        for step, (x, mel, mel_negative) in tqdm(enumerate(train_loader)):
            ...
            # Feed data
            x, mel, mel_negative = Variable(x), Variable(mel), Variable(mel_negative)
            if use_cuda:
                x, mel, mel_negative = x.cuda(), mel.cuda(), mel_negative.cuda()

            val_outputs, mel_encoded, mel_encoded_contrastive = model(mel, mel_negative)

            # Loss
            valence_loss = criterion(val_outputs, x)
            contrastive_loss = contrastive_criterion(mel_encoded, mel_encoded_contrastive)

            loss = valence_loss - contrastive_loss
        ...
        h.write("Loss after epoch " + str(global_epoch) + ': '  + format(running_loss / (len(train_loader))) + " Contrastive Loss: " + str(running_loss_contrastive / len(train_loader)) + " Valence Loss: " + str(running_loss_valence / len(train_loader)) + '\n')
        h.close()
        validate_model(model, val_loader)
...
if __name__ == "__main__":
    ...
    fnames_train = filenamesDataset(vox_dir +  '/fnames.train')
    fnames_val = filenamesDataset(vox_dir +  '/fnames.val')


    # Dataset and Dataloader setup
    trainset = ValenceDataset_CPCLoss(X_train, Mel_train, fnames_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=hparams.num_workers, shuffle=True,
        collate_fn=collate_fn_valence_contrastiveloss, pin_memory=hparams.pin_memory)
