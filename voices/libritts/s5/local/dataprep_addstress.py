...
feats_dir = vox_dir + '/festival/falcon_stress'
assure_path_exists(feats_dir)
desc_file = vox_dir + '/etc/falcon_feats.desc'
ids_file = vox_dir + '/etc/ids_stress.json'
...
for line in f:
 if len(line) > 2:
    ...
    phones_stress = ' '.join(k for k in line.split()[1:])
    phones = ' '.join(k.split('_')[0] for k in phones_stress.split())
    stress = ' '.join(k.split('_')[1] for k in phones_stress.split())
    stress_interpolated = get_interpolated_tones(phones, stress)
    if ctr % 100 == 1:
       print("Processed ", ctr, "lines")

    try:
      g = open(feats_dir + '/' + fname + '.feats', 'w')
      g.write(' '.join(k for k in stress_interpolated) +'\n')
      g.close()
...
g = open(desc_file, 'a')
g.write('stress|single|categorical' + '\n')
g.close()
