...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        running_loss = 0.
        for step, (mel, quants) in tqdm(enumerate(train_loader)):
            ...
            mel = Variable(mel).cuda()
            coarse = Variable(quants['coarse']).cuda()
            fine = Variable(quants['fine']).cuda()
            coarse_float = Variable(quants['coarse_float']).cuda()
            fine_float = Variable(quants['fine_float']).cuda()


            #print("Type of mels: ", mel.data.type())
            #print("Type of coarse: ", coarse.data.type())
            #print("Type of fine: ", fine.data.type())
            #print("Type of coarse_float: ", coarse_float.data.type())
            #print("Type of fine_float: ", fine_float.data.type())

            outputs = model(mel, coarse, coarse_float, fine, fine_float)

            # Loss
            #print("Shape of logits and targets: ", coarse_outputs.shape, coarse.shape)
            coarse_loss = criterion(outputs['coarse_logits'].contiguous().view(-1, 256), outputs['coarse'].contiguous().view(-1))
            fine_loss = criterion(outputs['fine_logits'].contiguous().view(-1, 256), outputs['fine'].contiguous().view(-1))
            ...
            if use_assistant and assistant is not None:
              ...
              assistant.log_scalar("Coarse Loss", float(coarse_loss.item()))
              assistant.log_scalar("Fine Loss", float(fine_loss.item()))
...
if __name__ == "__main__":
    ...
    if use_assistant:
       ...
       assistant = RemoteTracker(exp_name, projects_dir='sandbox')
