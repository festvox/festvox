...
from util import *
from model import CPCBaseline as Tacotron
...
if __name__ == "__main__":
    ...
    feats_name = 'quants'
    X_train = categorical_datasource( vox_dir + '/' + 'fnames.train', 
                                      vox_dir + '/' + 'etc/falcon_feats.desc', 
                                      feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name, 
                                      ph_ids)

    X_val = categorical_datasource( vox_dir + '/' +  'fnames.val', 
                                    vox_dir + '/' + 'etc/falcon_feats.desc', 
                                    feats_name, vox_dir + '/' +  'festival/falcon_' + feats_name, 
                                    ph_ids)

    # Dataset and Dataloader setup
    trainset = CPCDataset(X_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=hparams.num_workers, shuffle=True,
        collate_fn=collate_fn_quantsCPC, pin_memory=hparams.pin_memory)

    valset = CPCDataset(X_val)
