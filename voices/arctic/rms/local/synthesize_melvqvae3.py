...
#from util import *
from model import MelVQVAEv3 as Tacotron
...
def tts(model, mel):
    ...
    sequence = Variable(torch.from_numpy(mel)).unsqueeze(0)
    if use_cuda:
        sequence = sequence.cuda()

    mel_outputs,linear_outputs, alignments, vq_penalty, encoder_penalty, entropy = model.forward_eval(sequence)
...
if __name__ == "__main__":
    ...
    with open(text_list_file_path, "rb") as f:
        ...
        for idx, line in enumerate(lines):
            ...
            print(fname)
            #sys.exit()
            mel_fname = vox_dir + '/festival/falcon_mspec/' + fname + '.feats.npy'
            mel = np.load(mel_fname)
            waveform, alignment, _ = tts(model, mel)
