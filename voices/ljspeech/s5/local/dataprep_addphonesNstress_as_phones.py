...
feats_dir = vox_dir + '/festival/falcon_phonesNstress_as_phones'
assure_path_exists(feats_dir)
desc_file = vox_dir + '/etc/falcon_feats.desc'
ids_file = vox_dir + '/etc/ids_phonesNstress_as_phones.json'
...
def get_interpolated_tones(text, tones):
    ...
    for (phone, tone) in list(zip(text.split(), tones.split())):
        ...
        #for char in word:
        interpolated_tones.append(phone + '_' + tone )
...
g = open(desc_file, 'a')
g.write('phonesNstress|single|categorical' + '\n')
g.close()
