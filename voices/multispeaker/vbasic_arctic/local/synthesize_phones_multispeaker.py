...
def tts(model, text, spk):
    ...
    if use_cuda:
        ...
        spk = sequence.new(sequence.shape[0]).zero_() + int(spk)

    mel_outputs, linear_outputs, alignments = model(sequence, spk.long())
...
if __name__ == "__main__":
    ...
    with open(checkpoints_dir + '/ids_phones.json') as  f:
       phids = json.load(f)
    with open(checkpoints_dir + '/ids_speakers.json') as  f:
       spk_ids = json.load(f)
    phids = dict(phids)
