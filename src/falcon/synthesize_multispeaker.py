...
from models import TacotronOneMultispeaker as Tacotron
...
def tts(model,spk, text):
    ...
    # Greedy decoding
    mel_outputs, linear_outputs, alignments = model.forward_nomasking_multispeaker(sequence, spk)
    #print("Shape of output from the model: ", linear_outputs.shape)
...
if __name__ == "__main__":
    ...
    id2chars = {v:k for (k,v) in charids.items()}
    ...
    with open(text_list_file_path, "rb") as f:
      ...
      for idx, line in enumerate(lines):
         if len(line) > 2:
            ...
            spk = int(line.decode("utf-8").split()[1])
            text =  ' '.join(k.lower() for k in line.decode("utf-8").split()[2:])
            text = re.sub(r'[^\w\s]','', ''.join(k for k in text)) 
            words = nltk.word_tokenize(text)
            step = os.path.basename(checkpoint_path).split('.')[0].split('_')[-1]
            fname = 'spk' + str(spk) + '_' + fname + '_' + step
            print(text, fname)
            text = '< ' + text + ' >' 
            textcheck = []  
            for c in text:
            ...
            #sys.exit()
            print(' '.join(id2chars[k] for k in text))
            waveform, alignment, _ = tts(model, spk, text)
