...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.mcep_vits = kwargs.get('mcep_vits', False)
    ...
    self.enc_p = TextEncoder(n_vocab,
        ...
        p_dropout)
    if self.mcep_vits:
      mcep_order = kwargs.get('mcep_order', 39)
      mcep_alpha = kwargs.get('mcep_alpha', 0.455)
      mcep_n_fft = kwargs.get('mcep_n_fft', 1024)
      mcep_hop_length = kwargs.get('mcep_hop_length', 256)
      mcep_win_length = kwargs.get('mcep_win_length', 1024)
      mcep_kwargs = dict(
          initial_channel=inter_channels,
          mcep_dim=mcep_order + 1,
          n_fft=mcep_n_fft,
          hop_length=mcep_hop_length,
          win_length=mcep_win_length,
          alpha=mcep_alpha,
          gin_channels=gin_channels,
          decoder_channels=kwargs.get('decoder_channels', 192),
          n_decoder_resblocks=kwargs.get('n_decoder_resblocks', 5),
      )
      print('MCEP iSTFT VITS')
      from mcep_decoder import MCEPDecoder
      self.dec = MCEPDecoder(**mcep_kwargs)
    elif mb_istft_vits == True:
