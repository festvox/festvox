...
class TrainingDataPreparator:
    def __init__(self, phoneme_vocab_path: Optional[str] = None):
        self.phoneme_to_id = {}
        self.id_to_phoneme = {}
        self.max_audio_token = 0  # Track maximum audio token value
    ...
    def create_training_sequence(self, phoneme_ids: List[int], audio_tokens: List[int]) -> Dict:
        ...
        """
        # Track maximum audio token value
        if audio_tokens:
            self.max_audio_token = max(self.max_audio_token, max(audio_tokens))
    ...
    def prepare_dataset(self, phonemes_jsonl: str, codec_dir: str, 
        ...
        for i, data in enumerate(phoneme_data):
            ...
            codec_filename = audio_filename.replace('.wav', '.wav.npy')
    ...
    def save_dataset(self, train_examples: List[Dict], val_examples: List[Dict], 
        ...
        # Save vocabulary
        audio_offset = len(self.phoneme_to_id) + len(self.special_tokens)
        total_vocab_size = audio_offset + self.max_audio_token + 1  # +1 because tokens are 0-indexed
        
        vocab_info = {
            'phoneme_to_id': self.phoneme_to_id,
            'id_to_phoneme': self.id_to_phoneme,
            'special_tokens': self.special_tokens,
            'audio_token_offset': audio_offset,
            'max_audio_token': self.max_audio_token,
